...
// cobolscriptは処理結果をconsole.logに出力する仕様であるため、出力結果をAlexaに渡すためにフック処理を行う
...
const QuestionIntentHandler = {
    ...
    handle(handlerInput) {
        ...
        // COBOLソースからJavaScriptへトランスパイル
        var program = cobolscript.compileProgramFile(SCRIPT);
        program.run(cobolscript.getRuntime());
        
        // COBOLソース中でDISPLAYされた出力から、メッセージをランダムに選択
        var index = Math.floor(Math.random() * logs.length);
        
        const speakOutput = logs[index];
        return handlerInput.responseBuilder
            .speak(speakOutput)
